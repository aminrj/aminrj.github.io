...
The MCP specification places tool descriptions outside its trust boundary, but provides no mechanism for hosts to enforce that boundary. I didn't discover this gap — the spec says it plainly, in the Security and Trust and Safety section, version 2025-11-25: *"Descriptions of tool behaviour should be considered untrusted, unless obtained from a trusted server."* It then defines no mechanism for attesting a trusted server, verifying a description has not changed since approval, or enforcing any integrity control on the description field.

A year of production data shows what happens when you specify a security property but do not enforce it: thirty CVEs, one confirmed in-the-wild incident, and a disclosure last month that hit 200,000 instances. The vendor's response was: working as intended.

Three attack classes. The controls that work. The controls that are commonly recommended but insufficient. Lab code for every attack is at [github.com/aminrj-labs/mcp-attack-labs](https://github.com/aminrj-labs/mcp-attack-labs).
...
The spec defines no server-of-origin field. The SDKs implement the spec correctly. The problem is what the spec leaves out, and what that omission enables.

---

## The empirical baseline

These are public disclosures with different methodologies. I am reporting lower bounds.
...
These are lower bounds because the methodologies differ and the data is not directly comparable. Simon Willison framed the structural condition last year: private data plus untrusted content plus an exfiltration vector. By default, every MCP deployment has all three.
...
The attack is simple. A tool description is free text with no length limit, no schema validation, no integrity check. The LLM ingests it as part of the system context at handshake, before the user types anything.
...
This isn't prompt injection. Prompt injection is about user input. This is metadata and configuration, part of the system context. The attack fires at handshake, not when the user submits a prompt. The user's maths question arrived after the exfiltration instruction was already in the model's context.

**Capability dependence.** Not every model follows the embedded instruction, and this matters.
...
Same agent. Same prompt. Same server. Temperature 0. Fifteen trials per model.

The larger model was more dangerous. MCPSecBench found the same pattern at scale: GPT-4.1 and Claude 3.5 Sonnet show higher poisoning attack success rates than smaller open models on identical attacks. Higher capability does not mean safer. It means the model is better at following instructions, including the ones it should not follow.

Zverev et al. at ICLR 2025 formally measured this: across all major model families, instruction-data separation fails consistently. The ASIDE paper replicated this in 2026 with five additional model families. You cannot fine-tune your way out of this.
...
The attacker does not need to be the only server. They just need to be one server in a setup with a flat namespace. The adversarial server holds no credentials for the trusted server, performs no authentication bypass, breaks no encryption. It just writes a description that tells the model to use tools it already has access to.
...
The detail that landed hardest in the room: end-to-end encryption is fully preserved. The exfiltration happens above the encryption layer, inside the trusted client. Signal would not help. The data leaves before it ever touches a wire.

There is also no log of the cross-server composition. In a flat namespace there is nothing to log, and the client has no record that one server's description caused a call to another server's tools.
...
The architectural finding does not depend on the number. The system had no mechanism to repair itself after trust was broken. The version-control model MCP inherits from npm was designed for code correctness, not trust continuity across server restarts. Fifteen clean reviews at install time meant nothing when the behaviour changed at runtime — that was the point.
...
**A researcher building mcp-drift**, a tool that tracks changes to MCP server tool descriptions, asked about injection techniques beyond the obvious patterns: "What can we inject in schemas that can bypass even the most heavily guardrailed model?"

The clichéd patterns ("IGNORE ALL PREVIOUS INSTRUCTIONS") are now caught by most capable models, so the more interesting space is indirect injection via schema fields other than `description`. The `inputSchema` object, parameter names, `required` arrays, `enum` values, even non-standard fields can carry instruction-bearing content that reaches the model's context. CyberArk's "Poison Everywhere" research, published December 2025, tested every field in a tool schema. No field was safe.

The second class that bypasses guardrails is instruction framing. The same semantic content has very different success rates depending on how it is framed: constraint, clarification, format specification, or continuation of an existing task. The `<IMPORTANT>` tag is one framing. There are others that are harder to pattern-match.
...
The implication of both: the attack surface is not the description field. It is the entire tool schema plus the tool's runtime output. Every byte that flows from an MCP server into the model's context is a potential instruction surface. Zero-trust for all external tool interactions is not paranoia. It is the correct architectural frame — and most teams are not operating from it.

---

## Controls: three tiers, honest coverage

I organised controls across three zones, because each attack operates in a different one.
...
Five of six attack classes have controls deployable today. The sixth requires protocol-level changes. Two institutional forums are working on this: the OWASP MCP Top 10 working group and the Agentic AI Foundation under the Linux Foundation.
...
The first is a cross-session rug-pull detection module. It persists description hashes across sessions in SQLite and compares against the original baseline, not the previous session. Gradual drift across ten sessions is invisible to current tooling unless you anchor against session one. The WhatsApp variant I described earlier would have been missed without it. The module will ship as part of [mcp-attack-labs](https://github.com/aminrj-labs/mcp-attack-labs).

The second is an MCP-to-A2A attack chain lab. A2A v1.0 formalised the agent-to-agent coordination layer above MCP in March 2026. Can an MCP foothold be used to abuse the A2A trust layer and propagate to other agents in a fleet? That is the question. The kill chain from initial MCP tool poisoning to lateral movement across an agent fleet via A2A capability spoofing has not been published as a practitioner lab — which is exactly what the project builds.

Both will be published with full source code and reproducibility notes when complete.

---

I'm happy to share slides, demo scripts, or run a private session for your team. Drop me a line at amine.raji.perso@gmail.com or [DM me on LinkedIn](https://linkedin.com/in/araji). Slides and full lab code are at [github.com/aminrj-labs/mcp-attack-labs](https://github.com/aminrj-labs/mcp-attack-labs). Subscribe to the [AI Security Intelligence newsletter](https://newsletter.aminrj.com) for weekly deep-dives that don't fit in a post.
