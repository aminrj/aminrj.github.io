# On July 28, MCP changes shape. Three attack surfaces open where your gateway can't see them.

   Jul 04, 2026    Issue #18

The new MCP spec goes final July 28 and moves security from the protocol to the endpoint. Plus: new research explains *why* prompt injection works, and it's not what you think. Here's what to check before the spec ships.
...
## This week in AI security[#this-week-in-ai-security](#this-week-in-ai-security)
...
Since launch, MCP has been stateful: an agent connects, gets an `Mcp-Session-Id` in the HTTP header, and every request carries it. The model never saw that ID, never touched it, so it could never leak it. The new spec removes the handshake entirely. Now when an agent calls something like `create_browser()`, the server returns a handle *inside the conversation*: `{ "browser_id": "browser-abc123" }`. The model sees it, holds it, and passes it back on every call. Akamai and Backslash both broke down what that opens up, and it's three distinct things. **Handle hijacking** replaces session hijacking: the handle is now just a string sitting in the chat transcript, a Jira ticket, a Slack paste, or a tool response, and whoever can read or plant that string can replay it unless the server validates the handle against the auth context, not the handle alone. **The filesystem scope gap**: the old `Roots` capability, the one structural boundary on what paths an agent could touch, is deprecated. Scope enforcement is now opt-in, implemented per-server-developer, applied inconsistently. **IDE HTML rendering**: the new MCP Apps extension (SEP-1865) lets servers ship interactive HTML that renders in a sandboxed iframe *inside your IDE*, next to your source, terminal, and every connected server, which turns UI mimicry (a fake VS Code auth prompt) into a credential-harvesting surface.
> **The unifying point, and the one to sit with: all three of these are invisible to network and gateway security, by design.** A handle string in a conversation isn't HTTP traffic. Filesystem access on a developer laptop never crosses the gateway. HTML rendering in an IDE iframe generates no network signal. Your existing MCP gateway wasn't built badly, it was built for a different threat model, and the new spec relocates the risk to the endpoint where the gateway has no visibility. This is good engineering, to be clear. Stateless scales, and the OAuth 2.1 hardening (mandatory PKCE, audience-bound tokens, `iss` validation per RFC 9207) genuinely closes real holes. But the security work that used to live at the session layer now has to happen on the endpoint, explicitly, because you decided to do it.
> **Run these four checks before July 28, while you still have lead time.** One: for each MCP server you run, are handles tied to a specific user, and do they expire? A handle that never expires is a permanent access token. Two: inventory which MCP servers run locally on developer machines, because those are the ones no network tool will ever surface. Three: find which servers can render HTML into the IDE (MCP Apps) and set a review policy for that HTML the same way you'd vet a third-party script on your website. Four: stop assuming your gateway covers MCP, and decide deliberately where your endpoint visibility comes from. None of this requires migrating anything yet. It requires knowing what you're standing on before the ground moves.
...
This is the most clarifying piece of research I've read this year, and it presents at ICML in Seoul on July 6. "Prompt Injection as Role Confusion" (Ye, Cui, Hadfield-Menell) traces injection to a single mechanism: an LLM receives the whole conversation as one stream of text, partitioned by role tags (`user`, `tool`, `think`), and those tags are supposed to be the security boundary. But the model doesn't perceive role from the tag. It perceives role from *how the text sounds*. The researchers built probes that measure how strongly the model internally believes a token belongs to a given role, and found that reasoning-style text registers as the model's own private thoughts even when it's explicitly wrapped in `user` tags. Their attack, **CoT Forgery**, injects fake reasoning that mimics the model's own thinking style and takes jailbreak success from near-zero to about 60% across every frontier model tested. The same mechanism explains ordinary agent injection: prepend "User:" to a malicious command buried in a webpage and the model treats it as a trusted instruction. Across 1,000 exfiltration attempts, attack success rose from 2% to 70% in lockstep with how much role confusion the probe measured, *before a single token was generated*.
> **This is the mechanistic proof of something I keep saying: you cannot filter your way out of prompt injection, because to the model, sounding like a role is indistinguishable from being one.** Every content filter, every "ignore previous instructions" detector, every guardrail that inspects the text is playing a game the architecture already lost, because the attacker isn't smuggling a keyword, they're impersonating a *voice*, and the model's own weights treat a convincing caricature of its reasoning as more authentically its-own than its real reasoning. The paper's quieter warning is worse for anyone building shopping or browsing agents: because role perception is a matter of degree, the mere *tone* of a retrieved page can bleed across the tag boundary and nudge an agent, no explicit injection required. An enthusiastic product page can steer a purchasing agent, testable at scale, legally.
> **Stop investing in "detect the bad prompt" as your primary defense and move the budget to "limit what a fooled agent can do."** Assume the injection will land, because the research says it will. The controls that actually hold are the ones downstream of the model's confusion: scoped permissions, a human gate on irreversible actions, per-request authorization that doesn't trust the model's interpretation of who's speaking. If you run reasoning models, treat any externally-sourced text that *looks like* structured reasoning as radioactive, that's the exact shape CoT Forgery exploits.
...
## Where this is going[#where-this-is-going](#where-this-is-going)
...
## From the lab[#from-the-lab](#from-the-lab)
...
## Tooling worth knowing[#tooling-worth-knowing](#tooling-worth-knowing)
...
## One thing to check this week[#one-thing-to-check-this-week](#one-thing-to-check-this-week)

Build your MCP inventory before July 28, because it's the prerequisite for everything else. One row per MCP server your team uses, four columns: (1) local or remote? (local = invisible to network monitoring, so it's your endpoint's job); (2) does it issue handles that are user-bound and time-limited, or handles that work forever for anyone? (3) can it render HTML into the IDE via MCP Apps? (4) is filesystem scope structurally enforced, or is it just a convention the developer promised to honor? You will not have clean answers for every server, and the blanks *are* the finding. This table is three weeks of lead time turned into a concrete backlog, versus finding out the hard way after the spec ships and the ecosystem moves.
...
## What I'm watching[#what-im-watching](#what-im-watching)
